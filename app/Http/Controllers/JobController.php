...
class JobController extends Controller
{
    ...
    public function store(JobRequest $request)
    {
        ...
        if ($request->hasFile('images')) {
            foreach ($request->file('images') as $index => $image) {
                ...
                $job->images()->create([
                    'image' => 'storage/'.$image->store('jobs', 'public'),
                ]);
            }
        }

        return $this->successResponse($job->load('images'), 'job', 'job_created_successfully');
    }

    public function index()
    {
        $query = JobAd::with('images');
        $jobs = $query->get();
    ...
    {
        $user = auth()->user();
        $query = JobAd::where('added_by', $user->id)->with('images');
        $jobs = $query->get();
